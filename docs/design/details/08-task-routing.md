...
- **構造マーカーの決定的ガード（受領の閉包補助・#175 恒久対策）**: 発話が依頼の構造マーカー（「依頼内容」「依頼名」「成果物」「対象ファイル」「完了条件」「拘束条件」「repo:」「docs:」— コード側の固定小リスト）を **2 個以上**含むとき、意図判定が chat と言っても **execute として扱う**。判定はマーカー文字列の存在検査のみ（LLM 不関与・P2 適合: 散文の意味解釈ではなく書式の存在確認）。**chat→execute の片方向のみ**で、逆方向（execute→chat）のガードは作らない — 雑談を実行へ誤昇格させる危険を増やさないため。execute へ倒しても実行が始まるわけではなく、契約化 → 入口ゲート → **着手確認（人の裁定面）** に出るだけなので、「誤って実行へ進む」非対称の原則とは両立する（誤ガードの実害は不要な着手確認 1 枚で、人は無視・訂正できる）。2026-09-16 実測: 構造化された実依頼（マーカー 5 個）が chat・confidence 0.98 で誤判定され、会話脳が「担当外」という根拠のない断り文を返して依頼が捨てられた — 意図判定は LLM 単独の関門（P1 未適用）であり、その揺らぎを構造の実測で受け止める
- **LLM 判定点の棚卸し検査（メタの閉包）**: 「LLM が何かを判定・生成する呼び出し点」をテストがソースから機械抽出し、既知リスト（裏取りの有無を注記）と突合する。リスト外の判定点が増えたら fail — 裏取りのない LLM 関門が無自覚に増える経路を、契約フィールドのスキーマ閉包と同じ型で塞ぐ（維持コストはリストの意図的な更新のみ）
