...
**検証は分解しない（出口ゲートの一本化）:** 依頼文が成果物欄や完了条件で「独立検証レポート」「レビュー結果」等を求めていても、検証を通常サブタスクとして起こさない。検証は出口ゲート（§8.10f・opus 固定でタスク末尾に自動発火）が担う段であり、サブタスク化すると (1) 出口ゲートと二重に走り、(2) execution × depth の写像（§2.2）にかかって軽量モデルへ落ち、検証の質が依頼文の書き方で変わる。担保は二層 — 分解プロンプトで「検証はサブタスクにしない」と指示し（事前）、分解結果から検証サブタスクを機械的に取り除く（事後・`plan.drop_verification_subtasks`）。除去の判定は出口ゲート固有の成果物名（独立検証・検証レポート・判定表・全件突合 等）の決定的照合のみで、LLM は関与しない（「テストを実行して報告する」等の正当な作業は落とさない）。落とした step に依存していた後続は、落とした step の依存を推移的に引き継ぐ。全件が検証サブタスクになる分解（依頼そのものが検証の依頼）は空のプランを作らないため入力をそのまま用いる。**依頼者が検証を書いても書かなくても、検証は出口ゲートの経路だけに落ちる。**
