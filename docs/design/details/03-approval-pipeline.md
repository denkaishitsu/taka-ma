...
### (5) 承認リクエストの判断材料（実体の添付・実測回答・TOCTOU 照合）

Tier 3 の承認リクエストは従来、操作文字列（`command`）と worker stdout の前後文脈（`context`）だけを提示していた。操作がスクリプト・ファイルの実行や参照を含む場合（例: `bash setup.sh`）、承認者に見えるのはパスだけで**中身が見えない** — 実運用で「この返答では判断は無理だ」となった（2026-09-14 実測）。読めない承認は実質ハンコであり、安全ゲートの空洞化である。人が判断できる材料を承認面に出す（P5 — 着手確認・入口ゲートと同じ原則の承認リクエストへの適用）。3 つの機構で塞ぐ:

**(a) 承認対象の実体の添付（機械採取）**

- 対象の特定は**決定的**に行う: `tool_input` の構造化データ（headless アダプタでは権威的・上記 (3)）からコマンド文字列をトークン分割し、workspace 相対または絶対パスとして**実在するファイル**だけを対象にする（構造⇄実測の照合のみ。散文からの推測・LLM 判定はしない）
- 実体は `cat -n`（行番号付き・1 ファイル上限・件数上限つき）で採取し、**Slack の承認リクエスト本文**と**承認レコード（承認ファイル）**の両方へ添付する。採取はコード側の固定コマンドのみ（LLM 出力をコマンドに接続しない — 出口ゲート・分解入力と同じ規律で、採取関数も共用する）
- 採取できない対象（バイナリ・上限超過・読み取り失敗）は「実体を添付できませんでした（未添付）」を明示する — 未添付を無印（添付済みの顔）で通さない
- interactive(pty) 由来は (3) の信頼境界どおり `tool_input` が権威的でないため、実体添付は best-effort とし、(3) の規定（Tier 2 以上・context 全体の提示）を変えない

**(b) 承認スレッドでの実測回答（probe_approval）**

- 保留中の承認リクエストのスレッドで人が質問したとき（「これは何をするスクリプトだ？」）、会話層は**承認レコードの実測ブロック**で応答する: 操作の逐語・`tool_input` の構造・添付済みの実体・帰属（task_id・サブタスク・契約の該当欄）・実体ハッシュ
- スレッド ts ⇄ 承認レコードの対応はコードが決定的に引く（承認リクエスト送信時に thread_ts をレコードへ記録済み）。LLM の役割は probe 種別の選択のみで、回答本文はコード組立（§8.3 probe・§8.10g 状態主張の規律の承認面への拡張 — 会話脳の記憶・言い換えで承認対象を説明しない）
- 保留中でないレコード（決着済み）への質問には決着状態（approved/rejected・押下時刻・TOCTOU 不一致でブロックした場合はその差分）を実測で返す
- **問答は複数回成り立つ**: 応答は 1 問ごとに承認レコードと現在の実測を読み直して組み立てる（ターン間に会話状態を持たない）。「何が変わった？」→ 記録⇄実測の差分で答える。「いつ・誰が変えた？」→ git の実測・ファイル監査（qu-e file_audit）の記録に**残っていれば**実測で答え、残っていなければ「記録が無く測れない」と返す（推測で変更の帰属を語らない — 状態主張は測定からのみ・§8.10g）。質問の受信は既存の会話受信規律（メンション付きスレッド返信）のまま

**(c) TOCTOU 照合（承認した実体 ⇄ 実行される実体）**

- 承認リクエスト作成時、(a) で特定した対象ファイルの **sha256 を承認レコードへ固定**する
- `approved` を検知して `Decision{allow}` へ変換する**直前**に同じ対象を再計測し、**1 件でも不一致なら allow を発行しない**（操作ブロック・fail-closed）。通知は帰属を明言し、**判断材料と次の行き先を含める**: (1) 「承認された内容と実行時点の内容が異なるため実行しませんでした」の宣言、(2) **承認時の記録 ⇄ 現在の実測の差分**（承認レコードに実体が残っているためコードが機械生成できる。「異なる」とだけ言って人に調べさせない — 判断材料を出さない通知は本節が塞ぐ当の欠陥の再演）。ただし通知に載せるのは**要約統計（変更ファイル名・±行数）と差分の先頭（上限つき）**までとし、**全文は正本ファイルへ保存してパスを併記**する（完了報告の結果ファイルと同じ規律。通知で人が判断するのは「この変更に心当たりがあるか」であり、全文の精読が要るときはファイルか下記 (b) のスレッド問答で掘る）、(3) 次の行き先の 2 択 — 変更が正当なら**現在の内容で新しい承認リクエストを出し直す**（旧承認の流用はしない）／意図しない変更なら**中止**して変更の調査へ
- ハッシュの採取・照合はコード側のみ。対象ファイルが 0 件の操作（ファイルを参照しないコマンド）は照合対象なし＝従来どおり（照合をすり抜けたのではなく、実体が承認対象に含まれないだけ — (a) の添付有無と対で明示される）
- 残存リスク: 照合できるのは (a) で決定的に特定できたファイルに限る。コマンド文字列に現れない間接参照（スクリプトが内部で読む別ファイル等）は対象外 — これは worker の逸脱として遵守照合・qu-e file_audit の多層が担う既存の分担を変えない
