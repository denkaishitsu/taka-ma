...
|----|------------|--------|---------|----------|
| ローカル | ya-ta 分解脳（Qwen3.6-27B）/ sa-ru 会話脳（Qwen3.6-35B-A3B）/ light（Gemma）/ qu-e（Qwen3.6） | Mac mini（ya-ta・sa-ru）/ MBP（light・qu-e） | あり（**実常駐=重み+KV** ＋同居モデル合計 ≤ ホスト RAM 予算） | config 更新 ＋ モデル pull ＋ サービス reload |
...
- 脳 LLM へ投入する履歴は従来どおり直近 N ターン（`MAX_HISTORY_TURNS`）に丸める（プロンプト肥大と応答劣化の防止。永続化はこの丸めと独立で、ファイル側も同じ上限で保持する）
...
**フォールバック:** 分解脳の出力が JSON としてパースできない場合、元の指示をサブタスク1件（category: heavy）として扱う。
...
| V-17 | タスク分解 | 複合指示を投入 → サブタスクに分解される | 分解脳（Qwen3.6-27B）が JSON 配列を返し、step/category/depends_on が含まれる |
