...
- [3. y/n 承認パイプライン設計](#3-yn-承認パイプライン設計)
  - [3.1 基本方針](#31-基本方針)
...
- [8. コンポーネント間通信仕様（IPC）](#8-コンポーネント間通信仕様ipc)
  ...
  - [8.5 ③ sa-ru → 対話型 worker CLI（重量タスク実行、PTY 経路）](#85-③-sa-ru--対話型-worker-cli重量タスク実行pty-経路)
...
- MBP上の全プロセスの起動・停止・管理
- PTYラッパーによるClaude Codeのstdin/stdout制御
- Claude Code の stdout から文脈を抽出し、ya-ta に判定を委譲
...
- **cross-review**: `:opus :gemini` のように 2 つ以上指定すれば cross-review として処理（専用キーワードなし）。**並行投入・結果収集は orchestrator が担当**（`asyncio.gather` で各モデルへ並行投入、各 heavy Semaphore を個別取得、部分成功許容、失敗モデルは Slack 通知）、**結果統合は ya-ta（DeepSeek-R1）が知的に実施**
...
- PTY（疑似端末）ラッパー方式でClaude Codeのstdoutを監視
- 三段階リスク判定による自動/半自動/手動承認

### 3.2 技術スタック

- **PTYラッパー**: `pexpect`（Python）でClaude Codeを子プロセスとして起動
- **パターン検出**: `expect([r'\[y/n\]', r'\(yes/no\)', pexpect.EOF])` でプロンプトを捕捉
- **stdin注入**: 判定結果に応じて `child.sendline('y')` or `child.sendline('n')` を送信
...
y/n 検出時の承認フローは 決定論の安全床（最優先）＋ 2 段階のリスク判定（2026-04-19 改訂、安全床を 2026-06-19 明文化）。

**(0) 静的安全床（決定論・Tier 判定前・最優先）**

ya-ta（LLM）が判定する**前**に、承認パイプラインが `pipeline.yaml` の `safety` リストと**決定論で**照合する。これは LLM の判定が誤った・乗っ取られた場合でも破壊的操作を通さない最終防壁であり、意図的に LLM を介さない（機械的・コード固定）。

- `safety.always_deny`（例: `rm -rf /` / `mkfs` / fork bomb）に一致 → **Tier 判定をスキップして即時 `n`（拒否）**。監査ログの `reason` に `always_deny` 該当を記録。
- `safety.always_escalate_to_human`（例: `sudo` / `deploy` / `production`）に一致 → スコープ・Tier 判定をスキップして **Tier 3（人間承認）** へ直行。
- どちらにも一致しなければ (1) スコープ判定へ進む。

> リストは `pipeline.yaml`（SSOT）で管理し、`ApprovalPipeline` が起動時にロードする。LLM 判定の grey zone（可逆/不可逆）のみ (1)(2) が担い、絶対防壁は本段で決定論的に守る二層構造。

**(1) タスク指示スコープ判定（安全床を通過した操作のみ）**

ya-ta が Tier 判定する前に、ユーザーの元の指示から「期待される操作」かを判定する。スコープ内であれば Tier に関係なく自動 y を送信し、ユーザーが指示した操作をユーザーに再確認しない。スコープ外の操作のみ Tier 判定（下記 (2)）へ進む。
...
ya-taが即座に判定し、`y` をstdinに流し込む。qu-eは不要。
...
flowchart TD
    ...
    B["sa-ru\nPTY stdout intercept"]
    B --> SF

    SF["承認パイプライン<br>静的安全床照合（決定論）<br>pipeline.yaml: always_deny / always_escalate"]
    SF --> SFD{"安全床に一致？"}

    SFD -->|"always_deny 一致"| ND["❌ n → stdin<br>即時拒否 (Tier 判定スキップ)"]
    ...
    SCD -->|"Yes (指示範囲内)"| YS["✅ y → stdin<br>自動承認 (Tier 判定スキップ)"]
    SCD -->|"No (範囲外)"| C
...
| # | コンポーネント | 概要 | 実行場所 |
|---|--------------|------|---------|
| 1 | sa-ru本体 | Gemma 4 12B常駐、PTYラッパー + プロセス管理（ログは各コンポーネント個別出力、中央集約器なし） | Mac mini |
...
容量適合判定の入力 `model_capacity.yaml` の `size_gb` は **実常駐（重み＋KV キャッシュ）** であり、推測値を入れない（CLAUDE.md）。値は実機測定で維持する。実測・記入・入替は「決定論だが進化する操作」のため**コード固定せずランブック化**（[`docs/sa-runbooks/model-capacity-and-swap.md`](../sa-runbooks/model-capacity-and-swap.md)）し、エージェントが deploy のたびに **Do→Check→Record** で更新する。コードに固定する不変条件は**容量不等式 `evaluate_swap` のみ**。
...
| 入力方式 | @メンション | 説明 |
|---------|-----------|------|
| `@taka-ma-sa-ru ...` | 必要 | チャンネルでのメンション。会話キューへ |
...
`source` は会話由来の `conversation`。file_audit の Approve/Reject ボタン経由のタスク（`slack_action`）は会話を経ず従来どおり直接 §8.3 (B) のタスクとして投入される（A1 §3「すべての操作・作業は同じ経路（§8.3）を通る」、§8.12）。`command` は生文ではなく sa-ru が固めた構造化要約。
...
- sa-ru が着手確認後に `status: "init"` で作成（会話由来）／ u-zu が `slack_action` で作成（file_audit 由来）
...
### 8.5 ③ sa-ru → 対話型 worker CLI（重量タスク実行、PTY 経路）

heavy 対話タスクで使用する **汎用 PTY 経路**。Claude Code / agy（Antigravity CLI — Gemini CLI の後継のコーディングエージェント）/ 将来の Codex 等、対話型 worker CLI 全般に共通の方式。

| 項目 | 仕様 |
|------|------|
| 方式 | SSH + PTY（pexpect + tmux）— **汎用 `WorkerPtyWrapper`** |
| 接続先 | `ssh mbp` |
| 起動コマンド | `ya-ta.yaml` の `models.<name>.command` から取得（`claude` / `agy` 等） |
| セッション管理 | tmux セッション `{task_id}-step{n}[-{model}]` |
| y/n 検知 | `pexpect.expect([r'\[y/n\]', r'\(yes/no\)', r'Allow\?', pexpect.EOF])` |

**対象モデル**: `ya-ta.yaml` で `methods` に `pty` を含むモデル全て（Claude Code 系 / Antigravity CLI / 将来追加される対話型 CLI）。

> **NOTE（agy の認証制約）**: agy の認証は macOS keychain 依存のため、素の SSH セッションからは認証情報を読めない（**SSH 直実行不可**）。**GUI セッション起源の tmux 経由でのみ実行可**（実測 2026-07-03）。agy を PTY 経路で使う場合、tmux サーバは GUI 起源であることが必要。

**実行フロー:**

```
1. sa-ru: ssh mbp "tmux new-session -d -s {instance_id} '{cli_command} {model_flag}' && tmux attach -t {instance_id}"
2. sa-ru: pexpect で tmux セッションにアタッチ
3. sa-ru: send_task("ログインフォームを実装して")
4. PTYラッパー: stdout を監視
5. y/n 検出 → ya-ta でリスク分類 → 承認パイプラインへ
6. 完了検出 → 結果を取得 → Slack に通知
```

**エラーハンドリング:**

- SSH 接続失敗 → 3回リトライ（10秒間隔）、失敗で `failed` + Slack 通知
- tmux セッション消失 → reconnect() で再アタッチ試行
- worker CLI 異常終了 → EOF 検出 → `failed` + Slack 通知
...
> **NOTE**: agy は対話型 heavy タスクにも対応可能で、その経路は §8.5（対話型 worker CLI、汎用 PTY 経路）に統合済。Gemini 3.1 Pro に関する用途別の参加（cross-review / fallback / 高度な解析 / 対話）の全体像は **§8.4.x 相互扶助機能** を参照。`ya-ta.yaml` の `gemini.methods: [pty, subprocess]` で両対応を宣言。
...
```
1. sa-ru: 承認ファイルを status=pending で作成
2. sa-ru: Slack に Block Kit 承認リクエストを送信
3. ユーザー: Slack で Approve/Reject ボタンをクリック
4. u-zu: ボタンイベント受信 → 承認ファイルの status を更新
5. sa-ru: ポーリングで status 変更を検知 → PTY に y/n 送信
```
...
| 項目 | 仕様 |
|------|------|
| 検知トリガ | watchdog（FSEvents、event-driven） |
| 判定対象 | 静的フィルタ通過分のみ（`ignore_patterns` ∪ `.gitignore`） |
| `.gitignore` 同期 | 判定実行時に mtime チェックで再読み込み |
| 判定者 | qu-e |
| 判定結果 | approve / deny / escalate |
| ノイズ抑制 | 同パス連続変更を集約 |
| 通知トリガ | deny / escalate のみ（approve はログ記録のみ） |
...
| 押下後経路 | §8.3 のタスク投入経路を再利用（専用経路は新設しない） |
| 保存形式 | jsonl |
...
| レコード ID | `id` フィールド（Slack ボタン callback で特定） |
...
| ボタン | 動作 | 主体 |
|--------|------|------|
| Approve | jsonl 追記 | qu-e |
| Reject | jsonl 追記 | qu-e |
| Reject | プロセス停止 | sa-ru（process_manager） |
| Reject | revert（ファイル変更取り消し） | ya-ta が振り分けた LLM |

押下後は §8.3 の経路で `/opt/taka-ma/data/tasks/` にタスクファイルを作成し、ya-ta が分解 → 振り分け先 LLM が実行する。
...
sequenceDiagram
    ...
    OC->>CC: SSH+PTY: "解析結果を踏まえて修正して"

    CC-->>OC: "Write to: src/app.ts [y/n]"
    OC->>GW: risk_classify("Write to: src/app.ts")
...
y/n 検出
  │
  ├─ Tier 1 (Low Risk) ──→ 自動 y 送信 ──→ 完了
  │
  ├─ Tier 2 (Medium Risk) ──→ qu-e レビュー
  │     ├─ approve ──→ 自動 y 送信 ──→ 完了
  │     └─ deny / escalate ──→ Tier 3 へ
  │
  └─ Tier 3 (High Risk) ──→ Slack 承認リクエスト
        ├─ Approve ──→ y 送信 ──→ 完了
        ├─ Reject ──→ n 送信 ──→ 中止
        └─ 5分タイムアウト ──→ 自動 n 送信 ──→ 中止
```
...
| V-04 | ya-ta リスク分類 | y/n 検出後にリスク分類が実行 | tier, reason が出力 |
| V-05 | Claude Code PTY 起動 | sa-ru → SSH → MBP で tmux セッション作成 | `tmux list-sessions` に `claude-*` が表示 |
| V-06 | y/n 検出 | Claude Code が y/n を出力 → sa-ru ログに検出記録 | PTYラッパーのログに `y/n detected` |
| V-07 | Tier 1 自動承認 | ファイル読み取り操作の y/n → 自動 y 送信 | 人間介入なしで実行続行 |
| V-08 | Tier 2 qu-e 審査 | ファイル書き込みの y/n → qu-e 呼び出し | SSH 経由で review_cli.py が実行され JSON 応答 |
| V-09 | Tier 2 → Tier 3 エスカレート | qu-e deny → Slack 承認リクエスト | Slack に Block Kit ボタンが表示 |
| V-10 | Tier 3 人間承認 | Slack で Approve ボタン → y 送信 | 承認ファイルの status=approved、実行続行 |
| V-11 | Tier 3 タイムアウト | 5分放置 → 自動 deny | status=timeout、n 送信、Slack に通知 |
...
4. sa-ru → SSH → MBP: Claude Code 起動（PTY経由）
5. Claude Code: "Write to: src/app.ts [y/n]"
6. PTYラッパー: y/n 検出
7. sa-ru → ya-ta: Tier 2 判定
8. sa-ru → SSH → MBP: qu-e レビュー
9. qu-e: approve → y 送信
10. Claude Code: 続行 → 完了
11. sa-ru → Slack: 結果通知
```
...
2. u-zu → タスクファイル作成
3. sa-ru → ya-ta: heavy 判定
4. sa-ru → SSH → MBP: Claude Code 起動
5. Claude Code: "Execute: deploy.sh --production [y/n]"
6. PTYラッパー: y/n 検出
...
4. sa-ru → SSH → MBP: Claude Code 起動（PTY経由、Opus）
5. Claude Code → 結果返却
6. sa-ru → Slack: 結果通知
```
...
4. orchestrator: Opus（Claude Code, PTY）と Gemini 3.1 Pro（Antigravity CLI, subprocess）を asyncio.gather で並行起動
...
4. Step 1: sa-ru → SSH → MBP: Claude Code 起動（PTY経由）→ 解析結果取得
5. Step 2: sa-ru → SSH → MBP: Claude Code 起動
   入力: "解析結果: {Step 1 の出力}\n上記を踏まえて改修して"
6. Claude Code: y/n → 承認パイプライン → 実行 → 完了
7. sa-ru → Slack: 結果通知
```
