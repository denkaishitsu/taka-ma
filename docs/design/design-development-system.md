...
`workspace` / `directive` / `constraints` / `acceptance` は会話⇄実行の受け渡し契約（§8.10f）。着手確認で人が承認した値が凍結されて載り、実行・検証はこのフィールドだけを読む（要約の散文から推測しない）。
...
| 却下 | Reject はタスク中止（上表）のため再投入が発生せず、引き継ぎの対象外 |
...
```
1. sa-ru: 意図が固まると要約を作り、契約化パス（§8.10f）で directive / constraints / acceptance /
   needs_repo を得て、ya-ta 分解でプランを得て、確認レコードを status=pending で作成
2. sa-ru: Slack に「要約 + 契約（workspace・directive・constraints・acceptance を常に明示）+
   計画プレビュー + 着手 / やり直す」ボタンを送信
...
新設は 3 フィールド。経路は `workspace` と同一（着手確認レコード → 確定タスク → dispatcher → worker / 検証）で、**新しい通信路は作らない**。
...
- 計画・workspace に加え、`directive`・`constraints`・`acceptance` を**常に**提示する（空なら「なし」と明示。見えていない契約は承認されない）。訂正経路（§10.2.1）で修正でき、承認で計画と一緒に凍結される
...
- **成果物 file 既定**: 依頼が**編集系の語**（作成・生成・追記・追加・編集・修正・更新・書く・保存・反映・直す等）と**成果物パス**（英字始まりの拡張子を持つ相対パスのトークン。絶対パス・URL 埋め込み・バージョン番号は対象外）を含むなら、当該パスの `file` 検査を契約へ**必ず**載せる。既存 `acceptance` に同一パスを対象とする検査があれば重複させず、付与は上限 5 件。削除系の語は編集系に含めない（削除依頼へ file 検査を課すと誤未達になる）。2026-08-25 実障害（成果物編集タスクが完了検査なしのまま「完了」と報告）の再発防止
