...
ya-ta は Mac mini 64GB に sa-ru と同居する。既定だとモデル上限（256K）のコンテキストで常駐し KV キャッシュが膨張する（旧 DeepSeek-R1 実測: 128K=常駐 47GB → 32K=26GB）。コードは HTTP API で num_ctx を渡さないため、**Step 2 の `ai_gateway.py` がモデルに `PARAMETER num_ctx 32768` を焼き込む**（同タグ上書き・重み共有・冪等）。手動操作は不要。値は [`ya-ta.yaml`](../../src/ai_gateway/config/ya-ta.yaml) の `num_ctx` と一致させる。
...
| 観点 | 成功 | エラー |
|------|------|--------|
| `単純:` の出力 | サブタスク 1 件、各サブタスクが `step` / `command` / `execution` / `depth` / `depends_on` を持つ | サブタスクが空、JSON パースエラー、キー欠落 |
| `複合:` の出力 | サブタスク 2 件以上、`depends_on` がリスト型で実行順を表現 | 1 件のまま分解されない、`depends_on` がリスト型でない |
| `execution` 値 | `inline` または `agent` のみ | `light` / `heavy` / `meta` など旧値が混じる |
| `depth` 値 | `shallow` / `deep` / `null`（省略）のいずれか | 他の値が混じる |
...
print('inline:', json.dumps(c.classify('このJSONをYAMLに変換して: {\\\"a\\\": 1}'), ensure_ascii=False))
print('agent:', json.dumps(c.classify('ログインフォームを実装して'), ensure_ascii=False))
print('agent(解析):', json.dumps(c.classify('プロジェクト全体のアーキテクチャを評価して'), ensure_ascii=False))
...
| 観点 | 成功 | エラー |
|------|------|--------|
| `inline:` の出力 | `execution: inline`（純生成 → gemma へ写像） | `agent` / 旧 `light`・`heavy` |
| `agent:` の出力 | `execution: agent`（実装 → depth deep → opus） | `inline` |
| `agent(解析):` の出力 | `execution: agent`, `depth: deep` | `inline` |
...
| 観点 | 成功 | エラー |
|------|------|--------|
| Slack 返却内容 | 分解・分類結果（`execution` / `depth` / `lane` / `model` / `methods` / `depends_on` / `confidence`）がメッセージで返る | Slack に何も返らない、または通常のタスク完了通知が返る |
...
> **検証概要**: ya-ta が Qwen3.6-27B を介してタスク分解・分類・リスク判定を実行し、execution × depth の 2 軸判定 → orchestrator の写像テーブル（ya-ta.yaml `routing.matrix`）によるモデル解決 / confidence < 閾値の迷い → sonnet 落下 / 昇格ラダー / cross-review / モデル指定解析 / `/exam_gw` ドライランが期待通り動作することを確認する。

| # | 検証項目 | 対応 |
|---|---------|------|
| 1 | ollama に分解脳モデル（ya-ta.yaml の `model`、現行 Qwen3.6-27B）が存在する | Step 1 |
...
| 4 | タスク分解: 複合指示が複数サブタスク（`step` / `command` / `execution` / `depth` / `depends_on`）に分解される | 動作確認 2 |
| 5 | タスク分解: `execution` が `inline` / `agent` のみ、`depth` が `shallow` / `deep` / `null` | 動作確認 2 |
| 6 | タスク分類: 「JSON→YAML 変換」→ execution: inline | 動作確認 3 |
| 7 | タスク分類: 「ログイン実装」→ execution: agent | 動作確認 3 |
| 8 | タスク分類: 「アーキテクチャ評価」→ execution: agent / depth: deep | 動作確認 3 |
| 9 | タスク分類: 曖昧なタスク → execution: agent / depth 省略（写像上 sonnet） | 動作確認 3 |
...
| シンボル | 実装ファイル | 役割 |
|---------|-------------|------|
| `TaskDecomposer` | [`decomposer.py`](../../src/ai_gateway/decomposer.py) | タスク分解（DAG 生成、設計書 §8.4 / §10.2）。生の 2 軸（execution × depth）を返す。JSON パースエラー時は元指示 1 件を `execution: agent`（写像上 sonnet）へ fallback。写像・迷いの落下・昇格は orchestrator の責務 |
| `TaskClassifier` | [`classifier.py`](../../src/ai_gateway/classifier.py) | execution × depth 判定 + モデル指定解析。`parse_model()` / `classify()` / `_build_capability_prompt()`（ya-ta.yaml の `models` から capability 判定セクションを動的生成） |
...
| ファイル | 役割 |
|---------|------|
| [`prompts/categories.md`](../../src/ai_gateway/prompts/categories.md) | 軸定義（execution × depth）。`decompose_task.md` / `classify_task.md` 両方が `{categories}` で参照する共通プロンプト。軸の判定基準を変えるときはここだけ修正 |
...
- [`src/ai_gateway/config/ya-ta.yaml`](../../src/ai_gateway/config/ya-ta.yaml) — モデル登録（opus / sonnet / haiku / gemini / gemma 等）、`routing.matrix`（execution × depth × confidence → モデル写像）、`routing.confidence_threshold`、`routing.escalation.ladder`、`fallback.max_fallback_attempts`
...
| ya-ta モデル | Qwen3.6-27B (Q4_K_M)（推論特化、Mac mini 64GB で ~18GB） |
| ルーティング方針 | execution × depth の 2 軸判定（設計書 §2.2）。迷い（confidence < 閾値・depth 省略）は写像テーブルで sonnet（中位・万能）へ落とす |
| 分類軸 | execution（inline / agent）× depth（shallow / deep / 省略）。旧 light / heavy の 1 次元は廃止 |
| モデル写像 | ya-ta は生の 2 軸のみ返し、orchestrator が `routing.matrix` で解決（関心の分離）。追加時は yaml + 分類プロンプト更新のみ |
| 難所フォールバック | 昇格ラダー `routing.escalation.ladder`（既定 `[haiku, sonnet, opus]`）。引き金は例外・タイムアウトと worker の `ESCALATE:` 自己申告。`fallback.max_fallback_attempts` で昇格段数制限 |
| 明示指定の挙動 | `:モデル名` 明示時は障害でも昇格しない（指定モデル尊重） |
...
入力: タスク記述 + モデル指定（あれば、from sa-ru）
  ...
  │
  │ (2) モデル指定なし → ya-ta (Qwen3.6-27B) が LLM で知的判定
  │     execution（inline / agent）× depth（shallow / deep / 省略）+ confidence
  │
  │ (3) orchestrator が routing.matrix でモデル解決
  │     inline&高conf→gemma / inline&低conf→haiku / agent&shallow→haiku /
  │     agent&deep→opus / 迷い(depth省略 or 低conf)→sonnet
  │
  └ (4) orchestrator が実行（昇格ラダー / cross-review 統合は構築手順書 05 参照）
```
