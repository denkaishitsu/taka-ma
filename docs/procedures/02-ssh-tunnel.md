...
- 相手機が到達不能でも `ssh <相手 alias>` は **10 秒で失敗**し、確立済み接続の無応答は約 30 秒で切れる（`ConnectTimeout 10` / `ServerAliveInterval 15` / `ServerAliveCountMax 2`。`BatchMode yes` で対話認証待ちも起きない）。理由は [是正記録 2026-09-04](../design/revisions/2026-09-04_mbp-unreachable-outage.md)。
