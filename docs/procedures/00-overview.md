...
- 構築に着手する前に、本システムが「**何で、どのマシンに何があり、どの LLM がどこで何のために動くのか**」を一望するためのページ。詳細仕様・設計判断は [設計書](../design/design-development-system.md)（部位別の詳細は [details/](../design/details/)）を参照。
...
| 10 | 可観測性・自動調整 | Observability LLM（監視・最適化・自動調整官）の検討と設計 — 稼働メトリクス／ログを LLM が監視し、リソース最適化・設定調整を提言・自動化する | 機能が出そろう過程で系の運用を自律化する土台。スケール前に観測性を確保 |
| 11 | スケール検証 | 複数ワークスペース運用（team_id 別トークン）の検証 | 機能が出そろってから多重運用を実証 |
| 12 | スケール検証 | Mac mini ↔ MacBook Pro の 10Gbps 直結による構築・実証 | ハードウェア最適化。独立項目 |
