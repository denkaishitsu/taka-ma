...
## 実装メモ（コード化の境界）
- **コード（不変条件・固定）**: `model_monitor.evaluate_swap`（容量不等式の検算。予算 − 最小余裕 `min_headroom_gb` を合格条件とする）。
- **コード（無人化のため昇格）**: 候補モデルの検証（一次ソース照合）・実測・適合判定・Slack 提示は
  [`src/ai_gateway/model_watch.py`](../../src/ai_gateway/model_watch.py) が無人で行う（設計書 §7.4 フロー表）。
  本ランブックの A は **deploy 時の現行構成の実測**、B は **承認後の入替**として残る。
- **ランブック（可変・本書）**: deploy 時実測・記入・入替。エージェントが実行し、Check と Record で正しさを担保。
- 無人化が必要になった操作だけを個別にコードへ昇格する（big-bang 改修はしない）。
