...
作業対象の git ブランチは発話中に `ブランチ: <名前>`（または `branch: <名前>`）で明示する（設計書 §8.10f 受け渡しフィールド）。

```
@taka-ma repo:/Users/xxx/DevDev/myproject ブランチ: fix-login バグを直して
```

- マーカー指定は**契約化脳の抽出より常に優先**される（決定的抽出・LLM 不関与）。依頼文中に別のブランチ名の言及（「repo main の docs/…」等）があっても、マーカーの値が勝つ
- 指定が**既存ブランチ**の場合: HEAD との不一致を実測し、切替（switch）が実行計画の先頭に機械付与される。着手確認に組立後コマンドが逐語表示される
- **新しいブランチで作業させたい**場合: 「ブランチ: fix-login を main から作成して作業」のように**作成まで指示**する（branch_create が計画に立つ。計画に載っているか着手確認で確認できる）。実在しないブランチ名を指定だけすると、切替が実行時に実測つき失敗で人へ返る（勝手に作成しない）
- **指定しない**場合: 「作る・変える」型の依頼は受付が台帳へ自動起票し、`task-<id>` ブランチを作成して作業を束縛する（§8.10h）。ブランチ名の指定は必須ではない
- ブランチ名は git 安全文字のみ・`..` 不可（fail-closed 検証）
