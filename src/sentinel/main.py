...
class TaskContextHandler(FileSystemEventHandler):
    ...
    def __init__(self, store: dict):
        """task_id → context dict を共有する store を受け取る。"""
        self.store = store
...
async def main():
    ...
    res_notify_interval = res_cfg.get("notify_interval_sec", interval)
    ...
    audit_observer = start_audit(config, reviewer, task_context_store, loop)

    # task_context 受信（§8.13）— watchdog で即時反映
    ctx_dir = config["task_context"]["dir"]
    os.makedirs(ctx_dir, exist_ok=True)
    ctx_handler = TaskContextHandler(task_context_store)
