...
def _enqueue_audit_action_task(action: str, record: dict, audit_log_id: str,
                               user: str, team_id: str) -> str:
    """file_audit Approve / Reject 押下を §8.3 経路でタスク投入する（A1 §3）。

    A1 §3「すべての操作・作業は同じ経路（§8.3）を通る」に従い、
    Approve / Reject 双方を §8.3 経路に投入。ya-ta が分解して各主体に振り分ける（A1 §3.1）。
    """
    if action == "approve":
        command = (
            f"audit_log_id '{audit_log_id}' の Approve を qu-e の jsonl に追記すること"
        )
    else:
        path = record["path"]
        command = (
...
def register_actions(app):
    ...
    def handle_approve(ack, body, say):
        ...
        if resolve_approval(request_id, "approved", user_id=user):
            say(f":white_check_mark: <@{user}> が承認しました (ID: {request_id})")
        else:
            say(f":warning: 承認できませんでした（既に処理済みか期限切れ） (ID: {request_id})")
    ...
    def handle_reject(ack, body, say):
        ...
        if resolve_approval(request_id, "rejected", user_id=user):
            say(f":x: <@{user}> が拒否しました (ID: {request_id})")
        else:
            say(f":warning: 拒否できませんでした（既に処理済みか期限切れ） (ID: {request_id})")
    ...
    def handle_exec_confirm(ack, body, say):
        ...
        if resolve_exec_confirm(exec_request_id, "confirmed", decided_by=user):
            say(f":rocket: <@{user}> が着手を承認しました")
        else:
            say(":warning: この確認は既に処理済みか、期限切れです")
    ...
    def handle_exec_reject(ack, body, say):
        ...
        if resolve_exec_confirm(exec_request_id, "rejected", decided_by=user):
            say(f":arrows_counterclockwise: <@{user}> がやり直しを選びました。会話を続けてください")
        else:
            say(":warning: この確認は既に処理済みか、期限切れです")

    @app.action("audit_approve")
    def handle_audit_approve(ack, body, say):
        """file_audit Approve 押下 — §8.3 経路で「jsonl 追記」タスクを投入（A1 §3）。"""
        ack()
        ...
        if record is None:
            say(f":warning: audit_log_id={audit_log_id} のレコードが見つかりません")
            return
        _enqueue_audit_action_task("approve", record, audit_log_id, user, team_id)
        say(f":white_check_mark: <@{user}> が approve しました (id: {audit_log_id})")
    ...
    def handle_audit_reject(ack, body, say):
        ...
        if record is None:
            say(f":warning: audit_log_id={audit_log_id} のレコードが見つかりません")
            return
        _enqueue_audit_action_task("reject", record, audit_log_id, user, team_id)
        say(f":x: <@{user}> が reject しました (id: {audit_log_id})")
