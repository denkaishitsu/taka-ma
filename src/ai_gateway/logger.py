...
class YaTaLogger:
    """ya-ta の light/heavy 判定を日付別 jsonl に追記するロガー。

    分類/分解時にモデルが下した生判定（カテゴリ・信頼度・理由）を残す。後段の
    Phase 2（誤判定パターン抽出 → 分類プロンプト改善）が振り返る一次データになる。
    """
    ...
    def log_decision(self, task: str, category: str, model: str,
                     reason: str, confidence: float, actual_result: str = ""):
        """1 件の判定を当日ログに追記する。

        Args:
            task: 判定対象のタスク指示文。
            category: 判定カテゴリ（light / heavy）。light→heavy 強制の前の生判定を残す。
            model: 指定された :モデル名（無ければ空文字）。
            reason: モデルが挙げた判定理由。
            confidence: 判定の信頼度。heavy 強制（< 0.8）の閾値判断にも対応する値。
